...
/**
 * @file test_tpuart_mac_physical.cpp
 * @brief Unit tests for generic TP1 MAC physical composition over TPUART backend adapter
 */
...
void setUp(void) {
    ...
    macPhysical = std::make_unique<Tp1MacPhysical>(std::make_unique<TpuartMediumBackendAdapter>(fakePhysical));
}
...
void test_init_send_receive_and_monitor_mode() {
    ...
    // In monitor mode, send is not supported.
    TEST_ASSERT_TRUE(macPhysical->sendFrame(frame).isError());

    TEST_ASSERT_TRUE(macPhysical->setBusMonitorMode(Toggle::Disable).isOk());
    TEST_ASSERT_EQUAL(2, fakePhysical.monitorCalls());
    TEST_ASSERT_EQUAL(static_cast<int>(Toggle::Disable), static_cast<int>(fakePhysical.lastMonitorMode()));

    TEST_ASSERT_TRUE(macPhysical->sendFrame(frame).isOk());
    TEST_ASSERT_EQUAL(frame.size(), fakePhysical.lastSent().size());

    fakePhysical.queueFrame(frame);
    auto rx = macPhysical->receiveFrame(0);
    TEST_ASSERT_TRUE(rx.isOk());
    TEST_ASSERT_EQUAL(frame.size(), rx.value().size());
    TEST_ASSERT_EQUAL(0, static_cast<int>(fakePhysical.lastTimeout()));
}
